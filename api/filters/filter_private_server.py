...
def filter_private_server(queryset, params):
    print(params)
    qf = Q()
    ...
    if q != '':
        ...
        qf = qf | Q(proxmox_default_volume__icontains=q)
        qf = qf | Q(proxmox_node_name__icontains=q)
        qf = qf | Q(proxmox_default_disk__icontains=q)
